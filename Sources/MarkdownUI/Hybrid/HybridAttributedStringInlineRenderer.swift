...
// A copy of the inline attributed renderer that DOES NOT resolve fonts, so we can
// keep FontProperties in the runs and map them to UIFont ourselves for UIKit.

extension InlineNode {
  func hybridRenderAttributedString(
    baseURL: URL?,
    textStyles: InlineTextStyles,
    softBreakMode: SoftBreak.Mode,
    attributes: AttributeContainer
  ) -> AttributedString {
    var renderer = HybridAttributedStringInlineRenderer(
      baseURL: baseURL,
...
private struct HybridAttributedStringInlineRenderer {
  ...
  private mutating func renderCode(_ code: String) {
    var attrs = self.textStyles.code.mergingAttributes(self.attributes)
    // Mark inline code so UIKit can choose whether to honor background
    attrs.markdownCodeInline = true
    self.result += .init(code, attributes: attrs)
  }
