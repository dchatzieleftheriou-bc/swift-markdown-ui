  ...
  var body: some View {
    DemoView {
      HybridMarkdown(self.content)
        ...
        .markdownBlockStyle(\.hybridCodeBlock) {
            hybridCodeBlock($0)
        }
        .markdownBlockStyle(\.codeBlock) {
          codeBlock($0)
        }
        .markdownCodeSyntaxHighlighter(.splash(theme: self.theme, useAttributed: skata))
    }
  }

    @ViewBuilder
    private func hybridCodeBlock(_ configuration: CodeBlockConfiguration) -> some View {
      VStack(spacing: 0) {
        HStack {
